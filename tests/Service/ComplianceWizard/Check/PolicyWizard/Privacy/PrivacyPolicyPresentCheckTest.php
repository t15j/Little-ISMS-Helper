...
final class PrivacyPolicyPresentCheckTest extends TestCase
{
    private DocumentRepository&MockObject $documentRepository;
    private PrivacyPolicyPresentCheck $check;

    protected function setUp(): void
    {
        $this->documentRepository = $this->createMock(DocumentRepository::class);
        $this->check = new PrivacyPolicyPresentCheck($this->documentRepository);
    }

    #[Test]
    public function testPassesWhenApprovedPrivacyPolicyExists(): void
    {
        ...
        self::assertSame('privacy_policy_present', $result->checkId);
        self::assertSame('gdpr', $this->check->getStandard());
    }

    #[Test]
    public function testFailsWhenNoApprovedPrivacyPolicy(): void
    {
        $tenant = $this->createMock(Tenant::class);
        $this->documentRepository->method('createQueryBuilder')
    ...
    public function testGapMessageActionable(): void
    {
        ...
        self::assertSame(
            'compliance_check.privacy_policy_present.fail_message',
            $result->gap['title'],
        );
