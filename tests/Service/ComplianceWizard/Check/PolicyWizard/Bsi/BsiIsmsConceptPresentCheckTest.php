...
final class BsiIsmsConceptPresentCheckTest extends TestCase
{
    private DocumentRepository&MockObject $documentRepository;
    private BsiIsmsConceptPresentCheck $check;

    protected function setUp(): void
    {
        $this->documentRepository = $this->createMock(DocumentRepository::class);
        $this->check = new BsiIsmsConceptPresentCheck($this->documentRepository);
    }

    #[Test]
    public function testPassesWhenIsmsConceptPublished(): void
    {
        ...
        self::assertSame('bsi_isms_concept_present', $result->checkId);
        self::assertSame('bsi', $this->check->getStandard());
    }

    #[Test]
    public function testFailsWhenIsmsConceptMissing(): void
    {
        $tenant = $this->createMock(Tenant::class);
        $this->documentRepository->method('createQueryBuilder')
    ...
    public function testGapMessageActionable(): void
    {
        ...
        self::assertSame(
            'compliance_check.bsi_isms_concept_present.fail_message',
            $result->gap['title'],
        );
