...
final class BsiTopLevelLeitliniePresentCheckTest extends TestCase
{
    private DocumentRepository&MockObject $documentRepository;
    private BsiTopLevelLeitliniePresentCheck $check;

    protected function setUp(): void
    {
        $this->documentRepository = $this->createMock(DocumentRepository::class);
        $this->check = new BsiTopLevelLeitliniePresentCheck($this->documentRepository);
    }

    #[Test]
    public function testPassesWhenApprovedLeitlinieExists(): void
    {
        ...
        self::assertSame(1, $result->details['approved_documents']);
        self::assertNull($result->gap);
        self::assertSame('bsi_top_level_leitlinie_present', $result->checkId);
        self::assertSame('bsi', $this->check->getStandard());
    }

    #[Test]
    public function testFailsWhenNoApprovedLeitlinie(): void
    {
        $tenant = $this->createMock(Tenant::class);
        $this->documentRepository->method('createQueryBuilder')
        ...
        self::assertSame(0, $result->details['approved_documents']);
    ...
    public function testGapMessageActionable(): void
    {
        ...
        self::assertSame(
            'compliance_check.bsi_top_level_leitlinie_present.fail_message',
            $result->gap['title'],
        );
